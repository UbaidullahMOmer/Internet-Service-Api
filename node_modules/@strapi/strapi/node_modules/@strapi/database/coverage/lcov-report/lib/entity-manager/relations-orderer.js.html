...
<head>
    <title>Code coverage report for lib/entity-manager/relations-orderer.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">lib/entity-manager</a> relations-orderer.js</h1>
        <div class='clearfix'>
...
const relationsOrderer = (initArr, idColumn, orderColumn, strict) =&gt; {
  const computedRelations = _.castArray(initArr || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>.map((r) =&gt; ({
    init: true,
    id: r[idColumn],
    order: r[orderColumn],
  }));
...
&nbsp;
    if (r.position?.before) {
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (relation.init) <span class="cstat-no" title="statement not covered" >r.order = relation.order + 0.5;</span>
      else r.order = relation.order;
      idx = _idx + 1;
...
<span class="cstat-no" title="statement not covered" >          throw new Error(</span>
     ...
     */
<span class="fstat-no" title="function not covered" >    ge</span>tOrderMap() {
<span class="cstat-no" title="statement not covered" >      return _(computedRelations)</span>
        .groupBy('order')
        .reduce(<span class="fstat-no" title="function not covered" >(a</span>cc, relations) =&gt; {
<span class="cstat-no" title="statement not covered" >          if (relations[0]?.init) <span class="cstat-no" title="statement not covered" >return acc;</span></span>
<span class="cstat-no" title="statement not covered" >          relations.forEach(<span class="fstat-no" title="function not covered" >(r</span>elation, idx) =&gt; {</span>
<span class="cstat-no" title="statement not covered" >            acc[relation.id] = Math.floor(relation.order) + (idx + 1) / (relations.length + 1);</span>
          });
